    ...
    public class HoSoService : BaseRepository, IHoSoService
    {
        ...
        public async Task<IEnumerable<DuLieuDuLichModel>> GetsHoSo(string langId, int linhvucId, FilterRequest request)
        {
            ...
            foreach (var item in data)
            {
                ...
                : item.LinhVucKinhDoanhId == (int)LinhVucKinhDoanh.DiSanVanHoa ? await _context.DanhMuc.Where(x => x.Id == item.LoaiHinhId && x.LoaiId == (int)LinhVucKinhDoanh.DiSanVanHoa).Select(x => new LoaiHinhModel() { Id = x.Id, TenLoai = x.Ten }).FirstOrDefaultAsync() : null;
            };
        ...
        public async Task<DuLieuDuLichModel> GetHoSo(int id)
        {
            try
            {
                ...
                item.LoaiHinh = item.LinhVucKinhDoanhId == (int)LinhVucKinhDoanh.CoSoLuuTru ? await _context.LoaiHinh.Where(x => x.Id == item.LoaiHinhId).Select(x => new LoaiHinhModel() { Id = x.Id, TenLoai = x.TenLoai }).FirstOrDefaultAsync()
                    ...
                    : item.LinhVucKinhDoanhId == (int)LinhVucKinhDoanh.LuHanh ? await _context.DanhMuc.Where(x => x.Id == item.LoaiHinhId && x.LoaiId == (int)LinhVucKinhDoanh.LuHanh).Select(x => new LoaiHinhModel() { Id = x.Id, TenLoai = x.Ten }).FirstOrDefaultAsync()
                    : item.LinhVucKinhDoanhId == (int)LinhVucKinhDoanh.VanChuyen ? await _context.DanhMuc.Where(x => x.Id == item.LoaiHinhId && x.LoaiId == (int)LinhVucKinhDoanh.VanChuyen).Select(x => new LoaiHinhModel() { Id = x.Id, TenLoai = x.Ten }).FirstOrDefaultAsync()
                    : item.LinhVucKinhDoanhId == (int)LinhVucKinhDoanh.DiSanVanHoa ? await _context.DanhMuc.Where(x => x.Id == item.LoaiHinhId && x.LoaiId == (int)LinhVucKinhDoanh.DiSanVanHoa).Select(x => new LoaiHinhModel() { Id = x.Id, TenLoai = x.Ten }).FirstOrDefaultAsync()
